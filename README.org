...
- :in-buffer ::
  Only show the template when the ~:in-buffer~ regexp matches the current buffer's name.
#+begin_src emacs-lisp :results scalar
(doct '(("Only in *scratch*" :keys "n" :file "" :contexts ((:in-buffer "^*scratch*$")))))
#+end_src

- :unless-buffer ::
  Only show the template when the ~:unless-buffer~ regexp does not match the current buffer's name.
#+begin_src emacs-lisp :results scalar
(doct '(("Except in *scratch*" :keys "n" :file "" :contexts ((:unless-buffer "^*scratch*$")))))
#+end_src

- :in-file ::
  Only show the template when the ~:in-file~ regexp matches the current buffer's file name.
#+begin_src emacs-lisp :results scalar
(doct '(("Only in work.org" :keys "n" :file "" :contexts ((:in-file "work\\.org$")))))
#+end_src

- :unless-file ::
  Only show the template when the ~:unless-file~ regexp does not match the current buffer's file name.
#+begin_src emacs-lisp :results scalar
(doct '(("Except in work.org" :keys "n" :file "" :contexts ((:unless-file "work\\.org$")))))
#+end_src

- :in-mode ::
  Only show the template when the ~:in-mode~ regexp matches the current buffer's major mode.
#+begin_src emacs-lisp :results scalar
(doct '(("Only in org-mode" :keys "n" :file "" :contexts ((:in-mode "org-mode")))))
#+end_src

- :unless-mode ::
  Only show the template when the ~:unless-mode~ regexp does not match the current buffer's major mode.
#+begin_src emacs-lisp :results scalar
(doct '(("Except in org-mode" :keys "n" :file "" :contexts ((:unless-mode "org-mode")))))
#+end_src

- :function ::
  Only show the template when function returns non-nil. The function is not passed any arguments.
#+begin_src emacs-lisp :results scalar
