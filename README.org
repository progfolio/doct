...
~%doct(KEYWORD)~ will insert the value declared with ~:KEYWORD~ in the declaration.
...
- :hook function ::
  - org-capture-mode-hook ::
    ...
    Runs FUNCTION when entering the org-capture-mode minor mode. Useful for running a
...
- doct-warn-when-unbound ::
  ...
  This can be overridden on a per-declaration basis by setting ~:warn~.
*** Contributing
