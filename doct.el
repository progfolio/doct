    ...
    (when values
      (pcase keyword
        ...
        ((pred (lambda (keyword) (member keyword doct-option-keywords)))
         (car values))
        (_ (if-let ((merge-fn (doct--custom-merge-function keyword)))
               (funcall merge-fn values)
             ;;@INCOMPLETE: what is a reasonable default case?
             ;;as it stands this eliminates the ability to use additive custom kewyords
             ;;unless the user has registered a custom merge function.
             (car values))))))
